...
void TestTransportCatalogueAddStop() {
    ...
    assert(tc.stopname_to_stop_.size() == 2);
...
void TestTransportCatalogueCompleteCatalogue() {
    ...
    assert(tc.stop_to_buses_.size() == 2);
...
void TestTransportCatalogueProcessingQueryStat() {
    {
        ...
        ReadQueryStat(q);
        assert(q.query_stat_.first == QueryTypeStat::StatBus);
